...
//		
		...
		try {
			if(userservice.checkCredenziali(idGenerico, password) != null) {
				if(userservice.checkCredenziali(idGenerico, password).isAdmin() == true) {
					
					if (submit.equalsIgnoreCase("amministratore")) {
						System.out.println("nell'if di amministratore login");
						RequestDispatcher rd = request.getRequestDispatcher("/areaPersonaleAdmin.jsp");
						rd.forward(request, response);
						return;
					}else{
						messageLogin="Sei un amministratore, devi fare Login Amministratore!";
						session.setAttribute("messageLogin", messageLogin);
					...
					if (submit.equalsIgnoreCase("utente")) {
						System.out.println("nell'else (utente login)");
						RequestDispatcher rd = request.getRequestDispatcher("/areaPersonale.jsp");
						rd.forward(request, response);
						return;
					}else{
						messageLogin="Sei un utente, devi fare Login Utente!";
						session.setAttribute("messageLogin", messageLogin);
						RequestDispatcher rd = request.getRequestDispatcher("/login.jsp");
						rd.forward(request, response);
						return;
